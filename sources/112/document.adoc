...
class LabelTemplate {
  ...
  attribute condition, LabelContextCondition {
    definition {
      Condition of the LabelContext for selecting the pattern, select conditions
      that return "true" first, then select the first pattern.
    }
    cardinality 0..1
  }
}
...
The `condition` attribute is a LabelContextCondition object that defines the
condition for selecting the pattern.

* If the condition is met, the pattern is selected.
* If multiple patterns have conditions that are met, the first pattern is
selected.
+
It is crucial to note that it is possible to create multiple matching
templates that have overlapping conditions.
+
[example]
====
A template with a `condition` of "element_count_gt" and a threshold of 1, and
another template with a condition of "element_count_eq" and a threshold of 2,
overlap when the count is equal to 2.
====
...
The LabelContextCondition class defines a context condition for a labeller to
determine which pattern to use.

In some labeling systems, the labeller may need to switch between different
conditions based on the defined thresholds.

[example]
====
In the ISO/IEC labelling system, a "NOTE" label takes on these forms depending
on the number of notes in a clause:
...
class LabelContext {
  ...
  //   definition "Labels that are assigned to content elements within this label context"
  //   collection true
  // }
}
----
...
The `parent` attribute is a reference to the parent label context to which this
label context relates.

The `element` attribute is a reference to the content element associated with
the label context.

The `labellers` attribute is a collection of Labeller objects that are bound to
this label context.

// The `targets` attribute is a collection of Label objects that are assigned to
// content elements within this label context.

// NOTE: The `targets` attribute is used for tracking the targets for content
// elements, in order to achieve label template differentiation based on count.
...
class Labeller {
  ...
  attribute template, LabelTemplate {
    definition {
      ...
      respected when generating the label. If there are more than one templates,
      the labeller should select the appropriate template based on the condition.
    }
    collection true
  }
  ...
  attribute global_template, LabelTemplate {
    definition {
      ...
      direction must be respected when generating the label. If there are multiple
      global templates, the labeller should choose the appropriate one based
...
----
+-------------------+           +-------------------+
| Annex Labeller    |  parent   | Figure Labeller   |
|-------------------|<----------|-------------------|
| id: "annex"       |           | id: "figure"      |
...
[example]
====
The following definition describes a labeller for tracking Notes in a Clause
with a conditional label template.
...
. A valid LabelStyle must be defined with appropriate Labeller objects.
. ContentElement objects must be correctly structured in a content tree.
. LabelContext objects must be properly associated with ContentElement objects.
. Each Labeller must have at least one valid LabelTemplate.
...
... If the Labeller has multiple LabelTemplate objects with conditions, evaluate each condition:

.... For each LabelTemplate, check if its LabelContextCondition evaluates to true for the current context.

.... Select the first LabelTemplate whose condition evaluates to true.

.... If no conditions evaluate to true, select the first LabelTemplate without a condition, or the first one if all have conditions
...
[appendix]
include::iso_iec_label_style.adoc[leveloffset=1]
