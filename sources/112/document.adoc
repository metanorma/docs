...
document structure and labeling context

=== label renderer

component that generates a human-readable label in string form from a label
object, which may vary depending on the usage context
...
* `Labeller`: Tracks sequence position within a label context
...
[source,lml]
----
instance JapaneseClauseLabelTemplate {
  pattern = "%parentの%n"
}
----
...
class Labeller {
  ...
  attribute global_template, LabelTemplate {
    definition "Template for generating the fully qualified, global label"
  }
