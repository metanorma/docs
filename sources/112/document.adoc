...
EDITOR: define the usage of the defined models and how they interact in various contexts. Consider including examples for clarity.
...
Must be associated with a Context object.

=== Algorithm for resolving the label for a content element

Given a ContentElement object, we want to get its label.

. From the ContentElement, find its applicable Context object. The applicable Context object is one that has a Labeller that can label the type of the ContentElement.

.. If the Context object is on it, use it.

.. If not, navigate through its `parent`
attribute subsequently until the closest Context object
is obtained.

. From the Context object, find the Labeller object that is
...
... If the LabelTemplate requires a sequence element, it will ask its Sequence object to generate a value for the position of the ContentElement in the Context.
